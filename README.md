...
- **The oracle is trusted at the point it reads a sensor.** Nothing on the ledger can tell an
  honest reading from an invented one, because the signature that matters is the oracle's and it
  is applied after the number exists. Anchoring proves the readings were not altered *afterwards*,
  which is a different guarantee. Closing this means the sensor signing its own readings, so the
  oracle relays evidence it cannot forge rather than evidence it authored.
