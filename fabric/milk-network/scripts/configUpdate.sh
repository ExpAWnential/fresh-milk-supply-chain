...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config ${MILK_NETWORK_HOME}/channel-artifacts/config_block.pb -o $(ordererAddress) --ordererTLSHostnameOverride orderer.example.com -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
