...
updateAnchorPeer() {
  peer channel update -o $(ordererAddress) --ordererTLSHostnameOverride orderer.example.com -c $CHANNEL_NAME -f ${MILK_NETWORK_HOME}/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
